...
[[nodiscard]] std::vector<xcb_window_t>
findNewScreens(const std::vector<xcb_window_t> &existing_screens) {
  ...
  for (; screens.rem > 0; xcb_screen_next(&screens)) {
    ...
    spdlog::info("Screen {} with root {}",
                 screens.index / xcb_screen_sizeof(screens.data), root);
  ...
  for (int i = 0; i < num_screens; i++) {
    ...
    if (xcb_ewmh_get_virtual_roots_reply(
      ...
      std::copy_if(
          reply.windows, reply.windows + reply.windows_len,
          std::back_inserter(new_screens), [&existing_screens](auto w) {
            return std::find(existing_screens.begin(), existing_screens.end(),
...
[[nodiscard]] std::vector<xcb_window_t>
mapChildren(const std::vector<xcb_window_t> &mRoots,
...
void monitorChildren(const std::vector<xcb_window_t> &children) {
  ...
  //    xcb_input_event_mask_t mask = {.deviceid = XCB_INPUT_DEVICE_ALL_MASTER,
  //                                   .mask_len = 1};
  //    xcb_input_xi_event_mask_t xiem = static_cast<xcb_input_xi_event_mask_t>(
  ...
  for (auto w : children) {
    //    xcb_input_xi_select_events(connection.get(), w, 1, &child_mask.mask);
    std::ignore = xcb_change_window_attributes_aux(
        connection.get(), w, XCB_CW_EVENT_MASK, &root_mask);
  }
}
...
getEventName(const std::weak_ptr<xcb_generic_event_t> &event) {
  if (auto locked_sp = event.lock()) {
    return xcb_errors_get_name_for_xcb_event(err_ctx.get(), locked_sp.get(),
                                             nullptr);
  }
...
bool windowIsNormalType(const xcb_window_t w) {
  ...
  if (!xcb_ewmh_get_wm_window_type_reply(ewm_connection.get(), window_type_req,
                                         &atom_reply, nullptr)) {
    return false;
  }
