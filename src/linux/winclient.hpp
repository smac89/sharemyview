 ...
 */
[[nodiscard]] static std::vector<xcb_window_t>
...
extern std::unique_ptr<xcb_connection_t, decltype(&xcb_disconnect)> connection;
extern std::shared_ptr<xcb_errors_context_t> err_ctx;

class WindowsClient {
public:
  explicit WindowsClient() { VERIFY((connection), "X connection is invalid"); }
  ...
  void start() {
    ...
    for (auto const &root : mRoots) {
      std::unique_ptr<xcb_query_pointer_reply_t> reply(xcb_query_pointer_reply(
          connection.get(), xcb_query_pointer(connection.get(), root),
          nullptr));
   ...
   * @return void
   * @details when the mouse leaves a window, we are notified, and we unset the
   * current window
   */
